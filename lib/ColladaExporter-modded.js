		...
		function processMaterial( m ) {
			...
			if ( matid == null ) {
				...
				if ( m instanceof THREE.MeshLambertMaterial ) {
					...
					let useVertexColor = m.vertexColors === true;
				...
				/*var techniqueNode = `<technique profile="COMMON" sid="common"><${ type }>` +

				var techniqueNodeExtra = m.side === THREE.DoubleSide ? `<extra><technique><double_sided sid="double_sided" type="int">1</double_sided></technique></extra>` : '' + */

				var techniqueNode = `<technique profile="COMMON" sid="common"><${ type }>` + 
				'...' + 
				`</${ type }></technique>`;

				var techniqueNodeExtra = m.side === THREE.DoubleSide 
				? `<extra><technique profile="MAYA"><double_sided sid="double_sided" type="int">1</double_sided></technique></extra>` 
				: '';
					...
					(
						...
						(
							useVertexColor ?
							'<color sid="diffuse">1 1 1 1</color>' : // white base to let vertex colors show
							(
								m.map ?
